...
# -----------------------------------------------------------
# 修改点 1: 修正账户类型逻辑
# R4 (分账户) 修正为 Fixed (不可循环提现)
# -----------------------------------------------------------
ACCT_TYPE_MAP = {
    'R1': 'Revolving', # 循环贷账户 -> 可循环
    'R2': 'Revolving', # 贷记卡账户 -> 可循环
    'R4': 'Fixed',     # 循环额度下分账户 -> 不可直接循环 (只进不出)
    'D1': 'Fixed',     # 非循环贷账户 -> 不可循环
    'R3': 'Fixed'      # 准贷记卡 -> 视为不可循环
}
...
class DebtSpiralSimulator:
    ...
    def run_month(self):
        ...
        if net_flow >= 0:
            ...
            # -----------------------------------------------------------
            # 修改点 2: 内部挖潜逻辑升级
            # 1. 必须是 Revolving (R1/R2)
            # 2. 剩余额度 > 0
            # 3. 剩余期限(maturity) > 0 (未到期)
            # -----------------------------------------------------------
            if gap > 1:
                for loan in self.loans:
                    # R4/D1 被 logic_type='Fixed' 拦截
                    if loan.logic_type == 'Revolving':
                        # 额外校验：剩余期限必须大于0
                        if loan.balance < loan.limit and loan.maturity > 0:
                            draw = min(loan.limit - loan.balance, gap)
                            loan.balance += draw
                            gap -= draw
                            self.log("填坑", "以贷养贷", draw, f"从 {loan.name} (剩{loan.maturity}期) 提现")
                            if gap < 1: break
...
# --- Main: 市场风控配置 ---
with st.expander("⚙️ 市场风控与模型参数配置", expanded=True):
    
    tab_global, tab_t1, tab_t2, tab_t3 = st.tabs(["🌍 全局策略", "🏦 T1 银行", "🏢 T2 消金", "🧨 T3 网贷"])
    
    with tab_global:
        col_g1, col_g2 = st.columns(2)
        with col_g1:
            toxic_block = st.checkbox("🚫 启用「T3污染阻断」", value=True, help="持有T3余额，银行(T1)拒贷")
        with col_g2:
            penalty = st.slider("📉 多头惩罚系数", 1.0, 2.0, 1.15)
...
if run_btn and selected_client_loans:
    ...
    fig_wall = px.bar(
        ...
        title="⚠️ 债务到期墙 - 本金偿还洪峰",
        labels={'x': '未来月份', 'y': '需偿还本金'},
        color=amounts, color_continuous_scale='Reds'
    )
    fig_wall.add_hline(y=income, line_dash="dash", line_color="#27ae60", annotation_text="月收入线")
    st.plotly_chart(fig_wall, use_container_width=True)
    
    st.divider()
    st.markdown("### 🌪️ 2. 动态死亡推演")
    ...
    res_t1, res_t2 = st.tabs(["📉 趋势图", "📋 审计日志"])
    
    with res_t1:
        df_h = pd.DataFrame(sim.history)
        if not df_h.empty:
            fig_stack = px.area(df_h, x="Month", y=["T1_Debt", "T2_Debt", "T3_Debt"], 
    ...
    with res_t2:
        ...
        st.dataframe(df_l.style.format({"金额变动": "{:,.2f}"}).map(lambda x: 'color:red' if x<0 else 'color:green', subset=['金额变动']), use_container_width=True)
